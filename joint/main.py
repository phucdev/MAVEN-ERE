...
def evaluate(model, dataloader, desc=""):
    ...
    causal_res = classification_report(causal_label_list, causal_pred_list, output_dict=True,
                                       target_names=CAUSAL_REPORT_CLASS_NAMES, labels=CAUSAL_REPORT_CLASS_LABELS)
    print("CAUSAL:", causal_res)
    result_collection["CAUSAL"] = causal_res
    subevent_res = classification_report(subevent_label_list, subevent_pred_list, output_dict=True,
...
if __name__ == "__main__":
    ...
    if not args.eval_only:
        ...
        scheduler = get_linear_schedule_with_warmup(bert_optimizer, num_warmup_steps=200,
        ...
        for epoch in range(args.epochs):
            for data in tqdm(train_dataloader, desc=f"Training epoch {epoch}"):
                ...
                for i in range(len(coref_scores)):
                    ...
                    pred_clusters, pred_event2cluster = get_predicted_clusters(prob)
                    gold_event2cluster = get_event2cluster(labels)
                    assert len(pred_event2cluster) == len(gold_event2cluster), print(pred_event2cluster,
                                                                                     gold_event2cluster)
                    eval_result = EvalResult(labels, gold_event2cluster, pred_clusters, pred_event2cluster)
                    coref_train_eval_results.append(eval_result)
                ...
                if glb_step % args.log_steps == 0:
                    ...
                    print("Train %d steps: coref_loss=%f temporal_loss=%f causal_loss=%f subevent_loss=%f" % (
                    glb_step, np.mean(coref_losses), np.mean(temp_losses), np.mean(causal_losses),
                    ...
                    print("TEMPORAL:", temporal_res)
                    causal_res = classification_report(causal_label_list, causal_pred_list, output_dict=True,
                                                       target_names=CAUSAL_REPORT_CLASS_NAMES,
                                                       labels=CAUSAL_REPORT_CLASS_LABELS)
                    print("CAUSAL:", causal_res)
                    subevent_res = classification_report(subevent_label_list, subevent_pred_list, output_dict=True,
                                                         target_names=SUBEVENT_REPORT_CLASS_NAMES,
                                                         labels=SUBEVENT_REPORT_CLASS_LABELS)
                    print("SUBEVENT:", subevent_res)

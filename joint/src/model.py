...
from transformers import AutoConfig, AutoModel, BertConfig, RobertaConfig, DebertaV2Config
import torch
...
class EventEncoder(nn.Module):
    def __init__(self, vocab_size, model_name="roberta-base", aggr="mean"):
        ...
        if isinstance(config, BertConfig) or isinstance(config, RobertaConfig) or isinstance(config, DebertaV2Config):
    ...
    def forward(self, event_embed):
        ...
        for i in range(len(event_embed)):
            ...
            if embed.size(0) <= 1:  # no event or only one event
                ...
                continue
            event1_id, event2_id = zip(
                *[(index, k) for index in range(len(embed)) for k in range(len(embed)) if index != k])
        ...
        for i in range(len(event_embed)):
            ...
            if embed.size(0) > 1:  # at least one event
                ...
                split_scores = [to_cuda(torch.tensor([]))] \
                               + list(torch.split(s_ij, [i for i in range(len(embed)) if i],
                                                  dim=0))  # first event has no valid antecedent
            else:
                split_scores = [to_cuda(torch.tensor([]))]
            epsilon = to_var(torch.tensor([[0.]]))  # dummy score default to 0.0
            with_epsilon = [torch.cat((score, epsilon), dim=0) for score in
                            split_scores]  # dummy index default to same index as itself
